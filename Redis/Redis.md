...
- volatile-lru：利用**LRU算法移除设置过过期时间的key** (LRU:最近使用 Least Recently Used )
- allkeys-lru：当内存不足以容纳新写入数据时，在**键空间中，移除最近最少使用的key（这个是最常用的）**
- volatile-ttl：从已设置过期时间的数据集（server.db[i].expires）中**挑选将要过期的数据淘汰**
- volatile-random：从**已设置过期时间的数据集**（server.db[i].expires）中**任意选择数据淘汰**
...
String是Redis最基本的类型，一个key对应一个value， **一个Redis字符串value最多可以是512M**

String类型是二进制安全的，以为这Redis的String可以包含任何数据，比如jpg图片或者序列化对象

**场景**：需要计数的场景，比如用户访问次数，热门文章的点赞，收藏，转发等等

### 数据结构

String的数据结构是 **简单动态字符串(Simple Dynamic String)**，是可以修改的字符串，采用 **预分配冗余空间的方式来减少内存分配**

![image-20220709085735469](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220709085735469.png)

内部当字符串分配的空间的capacity一般高于实际字符串的len

**扩容规则**：当字符串容量小于1M，每次扩容都是翻倍，当字符串容量大于1M，每次扩容都是增加1M

### 编码方式

对于`string`类型字符串，底层有3中编码方式`int`,`embstr`和`raw`

- `int`：当存储的全是数字，用`int`存储
- `embstr`:当存储的字符串长度<44字符，用`embstr`存储
- `raw`：存储字符串长度>44字符，用`raw`存储

> object encoding key 查看 `key`对应的`encoding`

## List

### 基本介绍

Redis列表是简单字符串列表，可以给插入顺序排序，可以天极爱一个元素到列表的头部或尾部，他是单键多值的

他的实际上是个 **双向链表，对两端的操作性很高，通过索引下标的操作中间节点的性能很差**

![image-20220709090646941](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220709090646941.png)

**应用场景**：发布和订阅，消息队列，慢查询

### 数据结构

List的数据结构为quicklist（快速链表）

- 首先在列表元素少的情况下会使用一块连续内存存储，这个结构是ziplist，也就是压缩链表，他们的元素是紧挨着的
- 当数据量多的时候会改为quicklist,因为普通链表需要附加的指针空间打，会比较浪费空间，比如列表只存int，但是还需要存prev和next

![image-20220709092330769](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220709092330769.png)

> Redis将链表和ziplist结合形成了quicklist，将多个链表用双向指针串联，这样既保证了快速插入，删除性能，又不会有太大的空间冗余
...
> pre指向前一个节点，next指向后一个节点，value保存当前对象的数据

![image-20220709093308651](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220709093308651.png)
...
- head指向头结点
- tail指向尾节点
- dup用于对节点value进行拷贝，一般 `=`就可以，但是特殊情况会用到节点转移函数
- free释放一节点的空间，即使用Redis自带的zfree进行释放
- `match`判断函数的两个值是不是相等的，相等返回1，不相等返回0
- len表示这个链表有多少节点

![image-20220709095241079](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220709095241079.png)
...
~~~c
type struct ziplist<T>{
    //压缩列表占用字符数
    int zlbytes;
    //最后一个元素距离起始位置的偏移量，定位最后一个节点
    int zltail_offset;
    //元素个数
    int16 zllength;
    //元素内容
...
![image-20220709100548348](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220709100548348.png)

`zltail_offset`可以定义到最后一个entry，然后开始倒序遍历，`ziplist`支持双向遍历
...
- prelen 保存前一个entry的长度，可以通过倒序遍历定位到上一个`entry`
- encoding保存content类型
- content保存元素内容，内联到`content`的内容

![image-20220709104418958](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220709104418958.png)



#### 问题：为什么有了LinkedList还需要ZipList

因为相对于linkedlist,ziplist少了prev和next指针，两个指针需要占用16字节，另外LinkedList的节点都是单独分配，加剧了内存的碎片化，影响内存管理效率。`zipList`是连续的内存，这样就节约内存

> ziplist遍历，首先通过`zlbytes`和`zltail_offset`定位到最后一个`entry`的位置，然后根据最后一个`entry`的`prelen`定位前面的`entry`

著作权归https://www.duktig.cn所有。 链接：https://www.duktig.cn/2022/01/21/redis底层数据结构/
...
![连锁更新](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/202111091740545.png)
...
#### quickList
 ...
 在Redis3.2版本之后，list底层多了一个实现方式`quickList`,构成:linkedlist+ziplist，它将`linkedList`按段切分，每一段使用`zipList`来紧凑存储，多个`zipList`之间使用双向指针串接起来

![image-20220709105320664](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220709105320664.png)
...
上面提到过，`quickList`中可以使用压缩算法对`zipList`进行进一步的压缩，这个算法就是**[LZF算法  (opens new window)](https://blog.csdn.net/u012319493/article/details/83653860)**，这是一种无损压缩算法，具体可以参考上面的链接。使用压缩算法对`zipList`进行压缩后，`zipList`的结构如下所示：
...
![img](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/202111091748141.png)
...
# etc.
list-compress-depth 0  `list-compress-depth`这个参数表示**一个`quickList`两端不被压缩的节点个数。**需要注意的是，这里的节点个数是指`quicklist`双向链表的节点个数，而不是指`ziplist`里面的数据项个数。实际上，一个`quicklist`节点上的`ziplist`，如果被压缩，就是整体被压缩的。
```
...
Redis hash是一个string类型的field和value的映射表，hash审核存储对象，类型Java的Map

**应用场景:** 系统中对象数据的存储

### 数据结构

Hash的底层是**zipList(压缩链表),dict(哈希表)**，当field_value长度短并且个数少用ziplist,否则用dict
...
- `type`和`private`两个属性是为了实现多态，字典中存储不同的值，对应的一些复制，比较函数不一样，就是为了多态的调用
- `ht[2]`,两个hash表
- `rehashidx` 是一个辅助变量，记录rehash过程，为-1表示没有rehash过程
- `iterators`:记录`dict`有几个迭代器遍历

#### **dictht**

dict本身是对dictht的简单封装，dictht的结构如下
...
- `table`是`dictentry`类型的数组，用于真正存储数据；size表示这个table的大小，sizemask用于计算索引位置
...
![image-20220711092446047](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220711092446047.png)

> `dict`结构体的示意图如上述所示：
>
> 上图是一个没有进行`rehash`的`dict`，整个`dict`有两个`dictht`,其中一个存储数据，另外一个为空

#### 扩容和缩容

哈希表元素增加，产生`hash`冲突的概率越大，`dict`通过 **拉链法**解决，`hash`冲突概率提升，链表会越来越长，这会导致效率下降

说到扩容和缩容，这和Java里面一样，这有个 **负载因子**，负载因子用于描述当前集合被填充的程度 ，Redis的`dict`负载因子=哈希表保存节点/哈希表的大小

##### 扩容规则

- 没有BGSAVE和BGREWRITEROF，负载因子>1扩容
- 有BGSAVE和BGREWRITEROF,负载因子>5扩容
- 负载因子小于0.1,`Redis`对哈希表收缩
...
Redis set对外提供和list是一个功能，但是set是可以自动去重的

Redis中set是string的无序集合，底层是**value为null的hash表**，所以添加，删除，查找的时间复杂度都是0(1)

**使用场景**: 基于set可以实现 **交集，并集，补集**的操作。Redis可以实现共同关注，共同喜欢等功能

### 数据结构

Set数据是dict字典，字典是通过hash表实现的,Java中的HashMap所有的value都指向一个对象，Redis的set也一样，内部提供了hash，只不过所有value都指向一个内部值

## ZSet

### 基本介绍

Redis有序集合和set类似，是一个没有重复元素的集合，但是关联了一个 **评分(score)**，评分按照最低分到最高分排序集合的成员， **集合成员唯一，但是评分可以重复**
...
SortedSet，一方面类似Java中的Map<String,Double>，可以给元素的value赋值一个权重`score`，另一方面类似`TreeSet`，内部元素对权重的`score`排序

zset底层用了两个数据结构：

1.hash表：hash作用关联元素value和分支score，保证value唯一性，可以通过value找到相同的score

2.跳跃表，跳跃表的目的是给元素进行排序，根据score获取元素列表

zset的实现`ziplist`和`skiplist`

> zipList：满足下列条件

- [score,value]键值对小于128个
- 元素长度小于64字节

> skipList:不满足上述条件，就使用skipList

- `hash`采用`key`和`value`映射，可以在`0(1)`找到`value`的值
- skipList **从小到大**储存分数
- skipList存储的是[score,value]键值对

### 跳跃表

对于有序集合的底层实现，可以用数组，平衡树，链表，数组不便于元素的插入，平衡树和红黑树效率高但是结构复杂，链表查询需要遍历，效率很低

Redis用的是跳跃表，增，删，查的复杂度为(OlogN),表笑形式简单，美中不足的是跳跃表占用空间打，**空间换时间**的思想

![image-20220711103950665](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220711103950665.png)

 ![image-20220711104523791](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220711104523791.png)

![image-20220711104950725](https://secondlife.oss-cn-qingdao.aliyuncs.com/img/image-20220711104950725.png)
...
默认持久化方式，将**内存数据以快照的方式保存到硬盘**，对应生成dump.rdb文件

#### 如何持久化

单独创建fork子进程

Redis单独**fork一个子进程进行持久化**，先将数据写进临时文件，等持久化结束，将当前的数据替换上次持久好的文件，此过程只有子进程进行I/O操作，**主进程仍然处理客户端的操作，确保性能**

**优点：**

- 只有一个dump.rdb方便持久化
- 容灾性好
- **RDB可以最大化Redis性能**，父进程做的就是fork子进程，然后继续接受客户端请求，让子进程负责持久化操作，父进程无需进行I/O操作。
- 比AOF更为效率

**缺点：**

- 数据安全性低，RDB是一段时间进行持久化，持久化之间发生故障就会数据丢失
- 每次RDB，父进程都会fork子进程，如果数据集庞大，fork子进程非常耗时

#### AOF:

用**日志形式记录Redis的每一个写操作**，将Redis执行过程的写指令记录下来，**只许追加文件不可更改文件**，**redis启动会读取appendonly.aof进行恢复数据**，默认不开启，把**appendonly从no改为yes来启动redis**

#### AOF重写

AOF文件一直被追加，会让AOF文件过于大，Redis用了重写机制

>  触发机制：Redis会记录上次重写AOF大小，如果当前AOF文件大小是上次的一倍并且大于64M，就会触发重写机制

#### 优点：

- 多种策略  1.每修改同步  2. 每秒同步 3. 不同步
- AOF是只进行追加操作的日志
- Redis在AOF过大提供重写机制，重写操作安全，即使宕机AOF不会丢失
...
**当Slave启动并连接到Master，会发送一个SYNC命令**,master此时将启动后台存盘进程，**将数据库快照保存在文件中，同时收集新的读写命令并且缓存起来，在后台执行写文件，完毕之后Master将数据库文件传送到Slave**,完成一次数据同步，Slave接收到数据库文件数据后将其存盘并加载到内存恢复数据库快照到slave上。

如果Master和Slave之间的链接出**现断连现象**，**Slave可以自动重连Master**，但是**在连接成功之后，一次完全同步将被自动执行**。

如果master同时收到**多个 slave发来的同步连接命令**，只会使用**启动一个进程来写数据库镜像**，然后发送给所有slave。
